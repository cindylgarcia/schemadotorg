...
class Select extends FlexFieldTypeBase {
  ...
  public function widget(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    if (!$this->getWidgetSetting('required') || !isset($items[$delta]->{$this->name})) {
      $options = ['' => '- Select -'] + $this->getWidgetSetting('allowed_values');
    }
    else {
      $options = $this->getWidgetSetting('allowed_values');
    }
  ...
  public function widgetSettingsForm(array $form, FormStateInterface $form_state) {
    ...
    $element['allowed_values'] = [
      ...
      '#rows' => 4,
      '#default_value' => $this->allowedValuesString($this->getWidgetSetting('allowed_values')),
  ...
  public function value(FlexItem $item) {
    if ($this->getFormatterSetting('render') == 'key') {
      ...
      $allowed_values = $this->getWidgetSetting('allowed_values') ? $this->getWidgetSetting('allowed_values') : [];
      return isset($allowed_values[$item->{$this->name}]) ? $allowed_values[$item->{$this->name}] : '';
    }
  }
