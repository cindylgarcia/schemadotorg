...
class FlexItem extends FieldItemBase {
  ...
  public function isEmpty() {
    ...
    foreach ($field_settings as $name => $settings) {
      // The check empty option was added later so for bc we're defaulting all
      // check_empty values to TRUE if they're not in the settings.
      $check = array_key_exists('check_empty', $settings) ? $settings['check_empty'] : TRUE;
      // If any of the empty check fields are filled, then we're definitely
      // not empty and we can just return false immediately.
      if ($check && $this->{$name}) {
        return FALSE;
      }
    }
  ...
  public function fieldSettingsForm(array $form, FormStateInterface $form_state) {
    ...
    $elements['field_settings'] = [
      '#type' => 'table',
      '#header' => ['', t('Type'), t('Settings'), t('Output Settings'), t('Check Empty?'), t('Weight')],
    ...
    foreach ($flexitems as $name => $flexitem) {
      ...
      $type = isset($field_settings[$name]['type']) ? $field_settings[$name]['type'] : 'text';
      $elements['field_settings'][$name]['type'] = [
        '#type' => 'select',
        '#title' => t('%name type', ['%name' => $name]),
      ...
      $elements['field_settings'][$name]['widget_settings'] = $flexitem->widgetSettingsForm($form, $form_state);
      $elements['field_settings'][$name]['formatter_settings'] = $flexitem->formatterSettingsForm($form, $form_state);

      $check_empty_default = isset($definition['check_empty']) ? $definition['check_empty'] : TRUE;
      $elements['field_settings'][$name]['check_empty'] = [
        '#type' => 'checkbox',
        '#title' => t('Check Empty?'),
        '#description' => t('When saving this field, if all FlexField items with this value checked are empty, the row will be removed.'),
        '#default_value' => isset($field_settings[$name]['check_empty']) ? $field_settings[$name]['check_empty'] : $check_empty_default,
      ];
      ...
      if (!empty($definition['never_check_empty'])) {
        ...
        $elements['field_settings'][$name]['check_empty']['#description'] = t('<em>This flexfield type can\'t be empty checked.</em>');
      }
